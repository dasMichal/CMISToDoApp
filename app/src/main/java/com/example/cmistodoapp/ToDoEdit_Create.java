...
public class ToDoEdit_Create extends AppCompatActivity implements LifecycleOwner
{
	...
	EditText todoNameLocationField;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		viewModel.getToDoObject_byID(toDoID).observe(this, v ->
		{
			...
			try
			{
				...
				if (!v.getLocation().isEmpty())
				{
					newtoDoEntityObject.setLocation(v.getLocation());
					todoNameLocationField.setText(newtoDoEntityObject.getLocation());
				}
		...
		viewModel.getSubtasksFromToDOTest(toDoID).observe(this, subtasks ->
		{
			...
			for (int i = 0; i < subtasks.size(); i++)
			{
				...
				System.out.println(subTask.isDone());
				newSubTextDatabase(subTask.getSubTaskText(), subTask.isDone(), subTask.getSubTaskID(), i);
			}
			newSubText(subTaskLayout.getChildCount());
	...
	private void init()         //Assign all the Objects in one Place
	{
		...
		todoNameLocationField = findViewById(R.id.todo_reminderLocation);
	...
	public void logic()
	{
		...
		todoNameTextField.setOnEditorActionListener((v, actionId, event) -> {
		...
		todoNameLocationField.setOnEditorActionListener((v, actionId, event) -> {

			if (actionId == 6)
			{
				Log.d("KeyEvent", "Enter Pressed");
				newtoDoEntityObject.setLocation(todoNameLocationField.getText().toString());
				viewModel.updateToDo(newtoDoEntityObject);
			}
			return false;
		});
	...
	private void showDatePicker(View textView, int whatTextView, int hourOfDay, int minute)
	{
		...
		DatePickerDialog.OnDateSetListener DataSetListener = (view, year, month, dayOfMonth) ->
		{
			...
			month++;    //Because apparently Java Calender saves Months from 0 - 11 WHY? Nobody knows
						// https://media.giphy.com/media/JTz7sQq2hC96sO3Qs3/giphy.gif
			...
			*/
			...
			Data datePackData = new Data.Builder()           //Very Smart name, I know.  Don't @ me
					...
					.putString("NotificationChannel", NotificationChannel)
					.build();
	...
	private void notificationPlaner(Data DatePackData, int year, int month, int dayOfMonth, int hourOfDay, int minute)
	{
		...
		Log.i("WorkManager", "Notification scheduled in " + delayToNotification.toHours()+":"+delayToNotification.toMinutes());
	...
	private void newSubText(int childCount)
	{
		...
		newSubText.setOnEditorActionListener((v, actionId, event) ->
		{
		...
		newSubText.setOnFocusChangeListener((v, hasFocus) ->
		{
	 ...
	 * Creates a new Sub-task TextField under the {@link EditText todoNameTextField}
	 *
	 * @param subTaskText The Text of the SubTask
	 * @param done Boolean if SubTask is done or not
	 * @param subTaskID The unique ID of the SubTask
	 * @param count  incrementing value in which position the SubTask is located
	 */
	private void newSubTextDatabase(String subTaskText, boolean done, int subTaskID, int count)
	{
		...
		subTaskCheckbox.setId(subTaskID);
		...
		subTaskCheckbox.setOnClickListener(v ->
		{
			...
			SubTask_Entity tmpSubTaskObject = new SubTask_Entity();
			tmpSubTaskObject.setSubTaskID(v.getId());
			tmpSubTaskObject.setFKToDoID(toDoID);
